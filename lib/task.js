...
class Task extends StateMachine {
  ...
  constructor (executor, options) {
    super([
      { from: undefined, to: Task.eState.PENDING },
    ...
    this.state = Task.eState.PENDING
    ...
    this.promise
      .then(() => {
        if (this.state !== Task.eState.CANCELLED) {
           ...
           */
          this.state = Task.eState.FULFILLED
           ...
           */
          process.nextTick(() => {
            this.state = Task.eState.RESOLVED
          })
        }
      })
      .catch(err => {
         ...
         */
        this.state = Task.eState.REJECTED
        process.nextTick(() => {
          this.state = Task.eState.RESOLVED
        })
      })
  ...
  process () {
    if (this.state !== Task.eState.RUNNING) {
       ...
       */
      this.state = Task.eState.RUNNING
   ...
   */
  cancel () {
    this.state = Task.eState.CANCELLED

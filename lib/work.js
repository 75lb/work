...
var E_STARTING = "starting", E_SUCCESS = "success", E_FAIL = "fail", E_TERMINATED = "terminated",
    ...
    E_CHILDRENCOMPLETE = "childrenComplete", E_DESCENDENTSCOMPLETE = "descendentsComplete", E_INFO = "info";
...
function Job(options){
    ...
    */
    this.define({ name: "name", type: "string", required: true, default: "unnamed" })
        ...
        .define({ name: E_DESCENDENTSCOMPLETE, type: "boolean" })
    ...
    this.on(E_MONITOR, function(job, evt, data){
        if ([E_ERROR, E_CHILDRENCOMPLETE, E_DESCENDENTSCOMPLETE].indexOf(evt) === -1) {
            ...
            cursor.write(sprintf(
                ...
                whiteSpace(job.getDepth()), job.name, job.state, job.errors.length, job.info.length, evt
            ));
            prevJob = job;
        } else {

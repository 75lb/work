...
var E_STARTING = "starting", E_SUCCESS = "success", E_FAIL = "fail", E_TERMINATED = "terminated",
    ...
    E_CHILDRENCOMPLETE = "childrenComplete";
...
function runSequential(){
    if (this.runOn && this.parent) console.log("debug", this.runOn, this.parent.state);
...
function enterStarting(){
    if (this.state !== S_RUNNING){
        ...
        throw new Error(
            util.format("invalid state transition (%s->%s)", this.state, S_RUNNING, this.toJSON())
        );
    }
...
Job.prototype.success = function(){
    if (this.state === S_RUNNING){
        ...
        throw new Error(
            util.format("invalid state transition (%s->%s)", this.state, S_SUCCESSFUL)
        );
    }
...
Job.prototype.terminate = function(){
    if (this.state === S_RUNNING){
        ...
        throw new Error(
            util.format("invalid state transition (%s->%s)", this.state, S_TERMINATED)
        );
    }
...
Job.prototype.ignore = function(){
    if (this.state === S_PENDING){
        ...
        throw new Error(
            util.format("invalid state transition (%s->%s)", this.state, S_IGNORED)
        );
    }

...
var E_STARTING = "starting", E_SUCCESS = "success", E_FAIL = "fail", E_TERMINATED = "terminated",
    ...
    E_CHILDRENCOMPLETE = "childrenComplete", E_DESCENDENTSCOMPLETE = "descendentsComplete", 
    E_PROGRESS = "progress", E_INFO = "info";
...
function Job(options){
    ...
    */
    this.define({ name: "name", type: "string", required: true, default: "unnamed" })
        ...
        .define({ name: "progress", type: "object" })
...
Job.prototype.setProgress = function(progress){
    ...
    this.event(E_PROGRESS, progress);
};
...
Job.prototype.monitor = function(){
    ...
    this.on(E_MONITOR, function(job, evt, data){
        if ([E_ERROR, E_INFO, E_CHILDRENCOMPLETE, E_DESCENDENTSCOMPLETE].indexOf(evt) === -1) {
            if (job === prevJob){
                cursor.previousLine();
            }
            printStateful(job.state);
            cursor.write(sprintf(
                "%s%-20.20s ✤ %-10s ✤ errors: %d ✤ info: %d ✤ %-10s\n",
                whiteSpace(job.getDepth()), job.name, job.state, job.errors.length, job.info.length, 
                evt === E_PROGRESS ? job.progress && job.progress.percentComplete : evt
            ));
            prevJob = job;
        } else {

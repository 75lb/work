...
function Job(options){
    ...
    */
    ...
    this.progress = null;
    this.info = [];
    this.on("tmp", function(){}); this.removeAllListeners();
    ...
    */
    ...
    this.on(E_MONITOR, function(job, eventName, info){
        if (self.parent) self.parent.emit(E_MONITOR, job, eventName, info);
...
function runParallel(){
    ...
    if(!this.runOn || this.runOn === this.parent.state){
        ...
        var firstChild = self.children[0];
        if (firstChild){
            self.on(E_COMPLETE, firstChild.run);
...
Job.prototype.starting = function(){
    this.emit(E_STARTING, this);
    this.emit(E_MONITOR, this, E_STARTING);
}
Job.prototype.success = function(){
    this.emit(E_MONITOR, this, E_SUCCESS);
    this.emit(E_SUCCESS, this);
    this.setComplete();
};
Job.prototype.fail = function(){
    this.emit(E_MONITOR, this, E_FAIL);
    this.emit(E_FAIL, this);
    this.setComplete();
};
Job.prototype.terminate = function(){
    this.emit(E_MONITOR, this, E_TERMINATED);
    this.emit(E_TERMINATED, this);
    this.setComplete();
};
Job.prototype.ignore = function(){
    this.emit(E_MONITOR, this, E_IGNORED);
    this.emit(E_IGNORED, this);
}
Job.prototype.inform = function(info){
    this.info.push(info);
    this.emit("info", info, this);
    this.emit(E_MONITOR, this, "info", info);
};
Job.prototype.setProgress = function(progress){
    this.progress = progress;
    this.emit(E_MONITOR, this, "progress");
    this.emit("progress", progress, this);
};
Job.prototype.setComplete = function(){
    if (this.progress) this.progress.percentComplete = 100;
    this.emit(E_MONITOR, this, E_COMPLETE);
    this.emit(E_COMPLETE, this);
};
Job.prototype.error = function(err){
    this.errors.push(err);
    this.fail();
    this.emit(E_MONITOR, this, "error", err);
...
Job.prototype.print = function(indent){
    ...
    l(
        "%s%s [runOn: %s, prog: %s, state: %s, comp: %s, err: %s]",
        padding, 
        this.name, 
        this.runOn || "", 
        this.progress ? this.progress.percentComplete : "", 
        this.state, 
        this.complete, 
...
Job.prototype.monitor = function(stream){
    if (stream.isTTY){
        cursor.eraseData(2);
        this.on(E_MONITOR, function(job){
            cursor.goto(1, 1);
